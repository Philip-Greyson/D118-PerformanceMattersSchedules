...
print(f"DBUG: SFTP Username: {sftpUN} |SFTP Password: {sftpPW} |SFTP Server: {sftpHOST}") # debug so we can see where oracle is trying to connect to/with
            ...
            with open('schedule_log.txt', 'w') as log: # open a file for log output first
                ...
                with open('pmschedules.txt', 'w') as outputfile:  # open the output file
                    ...
                    try:
                        ...
                        today = dt.now()  # get todays date and store it for finding the correct term later
                        # today = dt.now() + timedelta(days = 120) #testing purposes
                            ...
                            try:
                                ...
                                if str(student[1]).lower() not in badnames and str(student[2]).lower() not in badnames: #check first and last name against array of bad names, only print if both come back not in it
                                    ...
                                    if (status == 0): # only worry about the students who are active
                                        ...
                                        try:
                                            cur.execute("SELECT id, firstday, lastday, schoolid, dcid FROM terms WHERE schoolid = :school ORDER BY dcid DESC", school=schoolID)  # get a list of terms for the school. using bind variables as best practice https://python-oracledb.readthedocs.io/en/latest/user_guide/bind.html#bind
                                            terms = cur.fetchall()
                ...
                try:
                    with pysftp.Connection(sftpHOST, username=sftpUN, password=sftpPW, cnopts=cnopts) as sftp:
                        print('DBUG: SFTP connection established')
                        # print(sftp.listdir())  # debug
                        sftp.put('pmschedules.txt') #upload the file onto the sftp server
                        print("INFO: Schedule file placed on remote server for " + str(today))
                        print("INFO: Schedule file placed on remote server for " + str(today), file=log)
                except Exception as er:
